...
# --- ESIMD INT4 bridge: route AutoRound/INC INT4 through ESIMD GEMV kernels ---
cp /entrypoints/esimd_int4_bridge.py /opt/venv/lib/python3.12/site-packages/esimd_int4_bridge.py 2>/dev/null || cp /home/sero/qwen38-b70/esimd_int4_bridge.py /opt/venv/lib/python3.12/site-packages/esimd_int4_bridge.py
python3 << 'PYEOF4B'
init_path = "/opt/venv/lib/python3.12/site-packages/vllm/__init__.py"
with open(init_path, "r") as f:
    code = f.read()
if "esimd_int4_bridge" not in code:
    old = "import vllm.env_override  # noqa: F401"
    new = "import vllm.env_override  # noqa: F401\ntry:\n    import esimd_int4_bridge  # noqa: F401\nexcept Exception as e:\n    print(f'esimd_int4_bridge failed: {e}')"
    if old in code:
        code = code.replace(old, new, 1)
        with open(init_path, "w") as f:
            f.write(code)
        print("Added esimd_int4_bridge import to vllm/__init__.py")
    else:
        print("ERROR: could not find insertion point for esimd_int4_bridge")
else:
    print("esimd_int4_bridge already imported")
PYEOF4B
...
exec vllm serve /model \
  ...
  --tensor-parallel-size 2 --max-num-seqs 2 --max-num-batched-tokens 2048 \
