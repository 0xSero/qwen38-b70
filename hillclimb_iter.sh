...
for run in 1 2 3; do
  ...
  RESP=$(curl -s "$URL/v1/chat/completions" -H "Content-Type: application/json" \
...
# --- Coherence check ---
COHERENCE_RESP=$(curl -s "$URL/v1/chat/completions" -H "Content-Type: application/json" \
  -d "$(build_payload "What is 51 times 37? Answer with just the number." 16)")
COHERENCE_ANS=$(echo "$COHERENCE_RESP" | jq -r '.choices[0].message.content // ""' | tr -dc '0-9')
COHERENT="✅"
if [ "$COHERENCE_ANS" != "1887" ]; then
  COHERENT="❌"
  log "  COHERENCE FAIL: got '$COHERENCE_ANS' expected 1887"
fi
PARIS_RESP=$(curl -s "$URL/v1/chat/completions" -H "Content-Type: application/json" \
  -d "$(build_payload "What is the capital of France? One word." 8)")
PARIS_ANS=$(echo "$PARIS_RESP" | jq -r '.choices[0].message.content // ""' | head -c 20)
if ! echo "$PARIS_ANS" | grep -qi "paris"; then
  COHERENT="❌"
  log "  COHERENCE FAIL: got '$PARIS_ANS' expected Paris"
fi
log "  coherence: $COHERENT (51×37=$COHERENCE_ANS, capital=$PARIS_ANS)"
...
for i in $(seq 1 "$CONC"); do
  curl -s "$URL/v1/chat/completions" -H "Content-Type: application/json" -d "$CONC_PAYLOAD" -o "/tmp/warmup_${KNOB_NAME}_$i.json" &
done
wait
sleep 2
...
for round in 1 2 3; do
  ...
  TOTAL_TOKENS=0
  for i in $(seq 1 "$CONC"); do
    curl -s "$URL/v1/chat/completions" -H "Content-Type: application/json" -d "$CONC_PAYLOAD" -o "/tmp/resp_${KNOB_NAME}_${round}_$i.json" &
  done
  wait
  T1=$(python3 -c "import time; print(time.time())")
  for i in $(seq 1 "$CONC"); do
    N=$(jq -r '.usage.completion_tokens // 0' "/tmp/resp_${KNOB_NAME}_${round}_$i.json")
