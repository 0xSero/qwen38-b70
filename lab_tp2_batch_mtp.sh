...
# Patch xpu_communicator.py: CPU gloo fallback on OUT_OF_RESOURCES
python3 << 'PYEOF3'
...
if old in code:
    ...
    with open("/opt/venv/lib/python3.12/site-packages/vllm/distributed/device_communicators/xpu_communicator.py", "w") as f:
        f.write(code)
    print("Patched xpu_communicator.py: CPU gloo fallback")
else:
    print("xpu_communicator.py already patched or pattern not found")
PYEOF3

# MTP5 + multiple sequences for aggregate throughput
...
exec vllm serve /model \
  ...
  --tensor-parallel-size 2 --max-num-seqs 4 --max-num-batched-tokens 2048 \
