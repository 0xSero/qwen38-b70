#!/usr/bin/env bash
# launch-mtp-compile.sh — MTP + torch.compile (inductor) WITHOUT XPU Graph
# This gives inductor-optimized code that can handle dynamic MTP metadata
# without baking in fixed graph state.
# Usage: launch-mtp-compile.sh [spec_tokens] [port]
set -euo pipefail
...
# Write inner script to host tmp
ssh sero@omarchy "cat > /tmp/mtp_compile_inner.sh << 'INNEREOF'
#!/bin/bash
set -e
...
exec vllm serve /model \
  ...
  --spec-method qwen3_5_mtp --spec-tokens \$1
INNEREOF
chmod +x /tmp/mtp_compile_inner.sh"
...
docker run -d \
  ...
  -v /tmp/mtp_compile_inner.sh:/entrypoint.sh:ro \
  -p "$PORT":8000 \
  --entrypoint /bin/bash \
  "$IMAGE" \
